...
int EPoll::clientWrite(Client &client, Server &server) {
	...
	if (!client.hasRequestFailed())
		...
		response.addHeader("Content-Length", ws::itos(errorBody->getBodySize()));
		response.addHeader("Date", response.generateDate());
		response.setMessageBody(errorBody);
	}

	if (!server.sendData(client, response))
	...
	for (int i = 0; i < readyFdAmount; i++) {
		if (events[i].events & EPOLLERR) {
			...
			if (clientConnect(server) == -3)
				return -3;
        } else if (server.isConnected(events[i].data.fd)) {
