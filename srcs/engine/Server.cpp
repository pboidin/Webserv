...
bool Server::setup() {
    ...
    return startListening(10) && poller->pollFd(_socket.getFd(), poller->pollInEvent());
}
...
bool Server::sendData(Client &client, HttpResponse &response) {
	...
	if (write(client.getSocket().getFd(), responseStr.c_str(), responseStr.length()) == -1)
		return false;
	return true;
}

bool Server::connect(Client &client) {
	ws::log(ws::LOG_LVL_INFO, "[SERVER] -", "connected client with fd " + ws::itos(client.getSocket().getFd()) + " on server " + _serverInfo.getServerName());
